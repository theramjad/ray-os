...
11. **Use Obsidian as your notes/docs tool** — your vault is just markdown files on disk, so Claude can read, write, and link them directly. Backlinks and graph view let you spot patterns across notes. They added a CLI too yesterday making it easier for Claude Code to interface with it. May need a skill though.
...
21. **Defer searches to subagents aggressively** — I'm really aggressive in deferring things to subagents. I defer every search query to subagents to avoid filling up the context window. Sometimes Claude would want to search, does it in the main session then eats like 20% of the context window. Of course, you may lose some nuance in deferring them as the wrong information gets returned back to the main session, but I found this had gotten better because Claude often tells a subagent its reason for searching well. If you find it doesn't, tell it to.
![[images/50-tips/tip-defer-to-subagents/excalidraw_10.png]]
22. **Give every subagent a "why"** — I like my subagents to have a why for their existence. When a subagent knows *why* it's searching, it can ignore 90% of what it finds and focus on the signal. "Find how auth works" returns everything. "Find how auth works because we need to add rate limiting to the login endpoint" returns only the relevant middleware chain. This compounds when you throw 4-5 subagents at a plan — each with a clear why, their results fit together like puzzle pieces instead of 5 overlapping generic summaries that all cover the same surface-level stuff.
![[images/50-tips/tip-subagent-why/excalidraw_8.png]]
By default you may not see a why (show recent screenshot).
## Prompting
...
27. **Spec developer gives you peace of mind when vibe coding** — I like to do things that give me peace of mind. The spec developer helps you understand the code better — I can explain exactly how a specific feature works because I understand it via the questions it asked me.
